...
import { OffenseDetectionService } from './defense_detection.service';
...
export class OffenseDetectionController {
  ...
  /**
   * 🫰 THANOS SNAP - Delete detection by ID
   * Deletes the detection record and all associated images from both DB and MinIO
   */
  @Delete(':id')
  @HttpCode(HttpStatus.OK)
  async thanosSnap(@Param('id') id: string) {
    return await this.service.thanosSnap(id);
  }

  /**
   * 🫰 THANOS SNAP - Delete all detections by objId
   * Deletes all detection records with the given objId and their images from both DB and MinIO
   */
